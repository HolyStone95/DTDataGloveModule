...
/**
 * @file DTModule.cpp
 * @brief Implementation of the DTModule to control Blinking with Serial.
 */
...
bool DTModule::configure(yarp::os::ResourceFinder &rf) {
    /* Process all parameters from both command-line and .ini file */

    /* get the module name which will form the stem of all module port names */

    moduleName = rf.check("DTDataGlove",
                Value("/DTDataGlove"),
...
bool DTModule::respond(const Bottle& command, Bottle& reply) {
	...
	reply.addVocab32(REPLY_OK);
	mutex.post();
}

/* Called periodically every getPeriod() seconds */
bool DTModule::updateModule()
{
    return true;
}

double DTModule::getPeriod()
{
